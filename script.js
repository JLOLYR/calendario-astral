document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (El resto de funciones como addEventsToCell, createEventRow, downloadCalendarImage no cambian)
    ...
    function getDayClassification(dayData) { if (!dayData) return null; const ICON_GIF_PATHS = { tension: 'assets/aspects/tension.gif', friction: 'assets/aspects/friccion.gif', harmonic: 'assets/aspects/armonico.gif', creative: 'assets/aspects/creativo.gif', change: 'assets/aspects/cambio.gif', revelation: 'assets/aspects/revelacion.gif' }; const aspectCounts = { Trine: 0, Square: 0, Opposition: 0, Sextile: 0 }; if (dayData.Aspects) { const processed = new Set(); for (const p1 in dayData.Aspects) { for (const p2 in dayData.Aspects[p1]) { const key = [p1, p2].sort().join('-'); if (processed.has(key)) continue; processed.add(key); const aspectType = dayData.Aspects[p1][p2].type; if (aspectCounts.hasOwnProperty(aspectType)) { aspectCounts[aspectType]++; } } } } const slowPlanets = ['Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto', 'North Node', 'South Node']; if (dayData.Aspects?.Sun) { for (const planet in dayData.Aspects.Sun) { if (slowPlanets.includes(planet) && dayData.Aspects.Sun[planet].type === 'Conjunction') { return { title: 'Día de Revelación', iconPath: ICON_GIF_PATHS.revelation, colorClass: 'day-revelation' }; } } } if (aspectCounts.Square >= 4 || aspectCounts.Opposition >= 4) { return { title: 'Día de Tensión', iconPath: ICON_GIF_PATHS.tension, colorClass: 'day-tension' }; } if (aspectCounts.Square >= 2 || aspectCounts.Opposition >= 2) { return { title: 'Día de Fricción', iconPath: ICON_GIF_PATHS.friction, colorClass: 'day-friction' }; } if (aspectCounts.Trine >= 2 && aspectCounts.Square <= 1) { return { title: 'Día Armónico', iconPath: ICON_GIF_PATHS.harmonic, colorClass: 'day-harmonic' }; } if (aspectCounts.Sextile >= 2 || (aspectCounts.Sextile >= 1 && aspectCounts.Trine >= 1)) { return { title: 'Día Creativo', iconPath: ICON_GIF_PATHS.creative, colorClass: 'day-creative' }; } const hasPlanetChange = dayData.Sign_Changes?.some(change => change.planet !== 'Moon'); if (hasPlanetChange) { return { title: 'Día de Cambio', iconPath: ICON_GIF_PATHS.change, colorClass: 'day-change' }; } return null; }
    function handleLandingDayClick(cell, date, dayData) { if (selectedDayCell === cell) { navigateToDetailView(date); return; } if (selectedDayCell) { selectedDayCell.classList.remove('selected'); } cell.classList.add('selected'); selectedDayCell = cell; landingDayDetails.innerHTML = ''; currentDetailDate = date; let hasContent = false; const contentWrapper = document.createElement('div'); const classification = getDayClassification(dayData); if (classification) { const specialDayRow = document.createElement('div'); specialDayRow.classList.add('special-day-row', classification.colorClass, 'clickable-title'); specialDayRow.innerHTML = `<img src="${classification.iconPath}" alt="${classification.title}"><span>${classification.title}</span><img src="${classification.iconPath}" alt="${classification.title}">`; specialDayRow.onclick = () => showAndScrollToSymbol(classification.title); contentWrapper.appendChild(specialDayRow); } else if (countEvents(dayData) >= 4) { const warningIconPath = ICON_PATHS.alerts.Warning; const intenseDayRow = document.createElement('div'); intenseDayRow.classList.add('special-day-row', 'day-intense'); intenseDayRow.innerHTML = `<img src="${warningIconPath}" alt="Alerta"><span>DÍA INTENSO</span><img src="${warningIconPath}" alt="Alerta">`; contentWrapper.appendChild(intenseDayRow); } if (classification || countEvents(dayData) >= 4) { const separatorTop = document.createElement('div'); separatorTop.className = 'details-separator'; contentWrapper.appendChild(separatorTop); hasContent = true; } const specialEventsDiv = document.createElement('div'); if (dayData.Eclipse) { const eventName = `Eclipse ${dayData.Eclipse.type} ${dayData.Eclipse.subtype}`; const eventIconPath = ICON_PATHS.events[eventName]; if (eventIconPath) specialEventsDiv.innerHTML += `<div class="special-event-row"><img src="${eventIconPath}" alt="${eventName}"><span>${eventName}</span></div>`; } else if (FASES_LUNARES_PRINCIPALES.includes(dayData.Moon_Phase)) { const eventName = dayData.Moon_Phase; const eventIconPath = ICON_PATHS.events[eventName]; if (eventIconPath) specialEventsDiv.innerHTML += `<div class="special-event-row"><img src="${eventIconPath}" alt="${eventName}"><span>${eventName}</span></div>`; } if(specialEventsDiv.hasChildNodes()){ contentWrapper.appendChild(specialEventsDiv); hasContent = true; } const eventsContainer = document.createElement('div'); addEventsToCell(eventsContainer, dayData); if (eventsContainer.hasChildNodes()) { contentWrapper.appendChild(eventsContainer); hasContent = true; } if (hasContent) { landingDayDetails.appendChild(contentWrapper); const separatorBottom = document.createElement('div'); separatorBottom.className = 'details-separator'; landingDayDetails.appendChild(separatorBottom); const interpretationBtn = document.createElement('button'); interpretationBtn.textContent = 'Ver Interpretación'; interpretationBtn.classList.add('interpretation-btn'); interpretationBtn.onclick = () => navigateToDetailView(currentDetailDate); landingDayDetails.appendChild(interpretationBtn); } else { landingDayDetails.innerHTML = '<p class="initial-prompt">No hay eventos.</p>'; } }
    ...
    function countEvents(data) { let count = 0; if (!data) return 0; if (data.Moon) count++; if(data.Sign_Changes) count += data.Sign_Changes.length; if (data.Retrograde_Changes) count += data.Retrograde_Changes.length; if (data.Aspects) { const processed = new Set(); for (const p1 in data.Aspects) { for (const p2 in data.Aspects[p1]) { const key = [p1, p2].sort().join('-'); if (processed.has(key)) continue; processed.add(key); count++; } } } return count; }
